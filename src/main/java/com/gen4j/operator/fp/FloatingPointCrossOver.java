...
public final class FloatingPointCrossOver extends AbstractGeneticOperator<FloatingPointChromosome>
    ...
    public FloatingPointCrossOver() {
        super(0.65, ChromosomeCodeType.FLOATING_POINT);
    }
    ...
    public Pair<Individual<FloatingPointChromosome>, Individual<FloatingPointChromosome>> apply(
        ...
        if (length > 0) {
            System.arraycopy(parent1Value, crossOverPoint, offspringValue2, crossOverPoint + 1, length);
            System.arraycopy(parent2Value, crossOverPoint, offspringValue1, crossOverPoint + 1, length);
        }

        final double betha = random.nextDouble();
        offspringValue1[crossOverPoint] = betha * parent1Value[crossOverPoint] + (1-betha)*parent2Value[crossOverPoint];
        offspringValue2[crossOverPoint] = betha * parent2Value[crossOverPoint] + (1-betha)*parent1Value[crossOverPoint];

        Range range = parents.first().chromosome().range();
        return Pair.of(
